# visualization.py - Power BI Style Interactive Charts with Animations
...
def show_interactive_pygwalker(df):
    ...
    **Drag and drop to create charts like Tableau!**
    ...
    try:
        ...
        pyg_html = pyg.to_html(df, spec="./gw_config.json", use_kernel_calc=True)
...
def visualizations_sidebar(df):
    """
    Main visualization sidebar with PyGWalker as the primary option
    """
    st.sidebar.header("🖼️ Interactive Data Visualizations")
    st.sidebar.markdown("*Power BI style charts with animations & hover effects*")
    ...
    plot_type = st.sidebar.radio(
        ...
        key="visualization_type_radio"
    )
    
    # Add a separator and additional options
    st.sidebar.markdown("---")
    st.sidebar.markdown("### 🎨 Chart Enhancements")
    
    # Global chart settings
    if plot_type != "🚀 Interactive Dashboard (PyGWalker)":
        ...
        animate_charts = st.sidebar.checkbox("✨ Enable Animations", value=True, key="global_animate")
        show_toolbar = st.sidebar.checkbox("🛠️ Show Chart Toolbar", value=True, key="global_toolbar")
    
    # Render the selected visualization
    if plot_type == "🚀 Interactive Dashboard (PyGWalker)":
        show_interactive_pygwalker(df)
    elif plot_type == "📊 Animated Bar Chart":
        show_bar_chart_interactive(df)
    elif plot_type == "🥧 Interactive Pie Chart":
        show_pie_chart_interactive(df)
    elif plot_type == "📈 Statistical Histogram":
        show_histogram_interactive(df)
    elif plot_type == "🔵 Scatter Plot with Trends":
        show_scatter_plot_interactive(df)
    elif plot_type == "🔥 Correlation Heatmap":
        show_correlation_heatmap_interactive(df)
    elif plot_type == "📋 Statistics Dashboard":
    ...
    if st.sidebar.button("💾 Save Current View", key="save_chart"):
        st.sidebar.success("Chart saved! (Feature in development)")
    
    if st.sidebar.button("📧 Share Chart", key="share_chart"):
        st.sidebar.info("Sharing options coming soon!")
    
    # Chart tips
    st.sidebar.markdown("---")
    st.sidebar.markdown("### 💡 Pro Tips")
    st.sidebar.markdown("""
    - **Hover** over chart elements for detailed info
    - **Click & drag** to zoom into specific areas  
    - **Double-click** to reset zoom
    - **Use toolbar** for pan, zoom, and download options
...
def safe_plotly_chart(fig, use_container_width=True, key=None):
    ...
    try:
        ...
        st.info("Please try refreshing the page or selecting different data columns.")